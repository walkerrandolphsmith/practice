...
app.post("/topic", (request, response) => {
  ...
  if (!body.name)
    return response.status(400).send({ reason: "Topic requires a name" });
  if (!body.partitions)
    return response
      .status(400)
      .send({ reason: "Topic requires a number of partitions" });
...
app.post("/send", (request, response) => {
  const body = request.body;
  if (!body.topic)
    return response.status(400).send({ reason: "Requires a topic" });
  if (!body.message)
    return response.status(400).send({ reason: "Topic must have a message" });
    ...
    [
      {
        ...
        type: "BUY",
        messages: [JSON.stringify({ id: body.car, type: "BUY" })],
        partitions: 0,
      },
      {
        topic: "INVENTORY_CHANGE",
    ...
    [
      {
        topic: "INVENTORY_CHANGE",
        type: "SELL",
        messages: [JSON.stringify({ id: body.car, type: "SELL" })],
        partitions: 0,
      },
      {
        topic: "INVENTORY_CHANGE",
...
app.get("/car/:id", (request, response) => {
  ...
  return getCar(id)
    .then((messages) => {
      ...
      const numberOfPrebiousOwners = hydratedMessages.reduce(
        (count, message) => {
