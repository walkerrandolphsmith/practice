...
app.post('/divide', (request, response) => {
    ...
    if (denominator === 0) return response.status(400).send({ "reason": "denominator can not be zero" });
    if (denominator === undefined) return response.status(400).send({ "reason": "numerator is required in the request body" });
    if (numerator === undefined) return response.status(400).send({ "reason": "denominator is required in the request body" });
