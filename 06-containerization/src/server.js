...
app.get('/user', (request, response) => {
    ...
    if (username) {
        const match = Object.entries(usersById).find(([key, value]) => value.username === username);
        if (!match) return response.status(404).send({ reason: "A user with the specified ID was not found" });
        const [id, user] = match;
...
app.post('/user', (request, response) => {
    ...
    const isExistingUser = Object.values(usersById).some(user => user.username === username);
    if (isExistingUser)
        return response.status(409).send({ reason: 'User already exists' });
    const nextId = Number(Object.keys(usersById).reduce((acc, next) => acc < next ? next : acc, -1)) + 1;
